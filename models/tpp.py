...
class TPP(nn.Module):
    ...
    def log_prob(self, h, dt, markers, pi_z):
        """return ll (B, S - 1, 1)"""
        # padding mask
        ...
        if pi_z:
            pi, z = pi_z # (B, S, K)
            log_intn_ms = ((intn_ms * pi).log() * z).sum(dim=-1, keepdims=True)
    ...
    def loss(self, h, dt, markers, mask=None, pi_z=None):
        ll = self.log_prob(h, dt, markers, pi_z)
